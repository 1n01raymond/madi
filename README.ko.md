...
| 변경되지 않은 대형 모델의 재열기는 분이 아니라 초 단위입니다 | 동일한 839.9 MB 페더레이션의 381.4 s 콜드 임포트 대비, 새 프로세스 5회 웜 재열기 중앙값 1.43 s(`node` 시작 시간 포함) ([캐시 분포](artifacts/cache/sixty5/README.md)) |
| 기록된 가장 큰 모델도 별도 플래그 없이 임포트됩니다 | 기본 `scene.gltf`가 545,470,166바이트로 런타임 최대 문자열 길이보다 8,599,278바이트 크지만, 컴파일러가 문서를 스트림으로 기록하므로 컴파일됩니다 ([ADR-0016](docs/adr/0016-streamed-gltf-document.md), [캐시 분포](artifacts/cache/sixty5/README.md)) |
...
381.4초, 웜 1.36초, 손상 엔트리 폴백 89.0초의 중앙값을 기록했고, 15개 샘플
