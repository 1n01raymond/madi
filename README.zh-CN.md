...
| 重新打开未更改的大型模型只需数秒而非数分钟 | 相同 839.9 MB 联邦的冷导入为 381.4 s，而 5 次全新进程的热重开中位数为 1.43 s（含 `node` 启动）（[缓存分布](artifacts/cache/sixty5/README.md)） |
| 导入已记录的最大模型无需任何特殊参数 | 其默认 `scene.gltf` 为 545,470,166 字节，比运行时最大字符串长度多 8,599,278 字节，但因为编译器以流的方式写出文档而仍能编译（[ADR-0016](docs/adr/0016-streamed-gltf-document.md)、[缓存分布](artifacts/cache/sixty5/README.md)） |
...
381.4 秒、热恢复 1.36 秒、损坏条目回退 89.0 秒，15 个样本全部产生逐字节
