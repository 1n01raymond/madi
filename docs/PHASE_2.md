...
| Workspace reopen | Stable source/cache identities (**Implemented**) → workspace manifest and reopen decision (**Implemented**, [ADR-0022](adr/0022-workspace-manifest.md), off any host path) → Studio save/reopen wiring (**Implemented**) → Studio reopen evidence | A whole-package cache hit is not a saved workspace |
...
6. Record the Studio workspace round trip: save and reopen a session against an
   unchanged package, reopen the same workspace after a source edit, and show
   both in a browser. The format, the trust boundary, the reopen decision, and
   the Studio wiring that calls them all exist -- [`@naru3d/workspace`](../packages/workspace/README.md)
   under [ADR-0022](adr/0022-workspace-manifest.md), and the Studio's own
   capture, reopen, and source-inspection layer -- so what remains is the
   browser record that moves the exit criterion.
