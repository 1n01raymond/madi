...
1. Land the successor to the rejected payload tier that
   ...
   encoder/placement split and cache primitives stay). Remaining slices, in
   order: the three code slices (stored-byte verification, column-form
   structure, in-compiler federation assembly). The ADR's gate 4 is the same predeclared bar: a lower
