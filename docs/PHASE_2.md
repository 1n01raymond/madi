...
|---|---|---|
| Public engineering-package delivery | The repository half is implemented under proposed [ADR-0023](adr/0023-public-package-delivery-origin.md): a delivery-origin contract, origin-side digest verification in the deploy, a configurable default scene URL, and a smoke check that asserts the contract. The origin exists and serves Digital Hub (`https://packages.blacktanlabs.com/naru/digital-hub/v1/`, `NARU_PACKAGE_ORIGIN` set); the deploy verifies it, the smoke check passes from outside, and a [committed browser record](../artifacts/public-demo/digital-hub-origin/README.md) shows the deployed Studio opening it cross-origin to a first frame and a pick (ADR-0023 gates 1-3 met 2026-09-05); what remains is the qualified [31-document package](../artifacts/ifc/engineering-baseline/README.md) uploaded at its recorded digest, which at 854,446,743 bytes needs a multipart upload rather than the single-request path Digital Hub used; source/package qualification is already recorded | Complete Phase 2 public-scale exit and stronger ADR-0003 relevance |
