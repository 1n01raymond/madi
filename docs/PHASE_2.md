...
| Workspace reopen | Stable source/cache identities (**Implemented**) → workspace manifest and reopen decision (**Implemented**, [ADR-0022](adr/0022-workspace-manifest.md), off any host path) → Studio save/reopen wiring → Studio reopen evidence | A whole-package cache hit is not a saved workspace |
...
6. Carry the landed workspace manifest into the Studio: save and reopen a
   session against an unchanged package, reopen it again after a source edit,
   and record both. The format, the trust boundary, and the reopen decision are
   in [`@naru3d/workspace`](../packages/workspace/README.md) under
   [ADR-0022](adr/0022-workspace-manifest.md); what remains is host wiring and
   the browser record that moves the exit criterion.
