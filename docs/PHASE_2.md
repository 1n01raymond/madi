...
| Hierarchy and coarse preview during first import | 5–15 s | An already compiled sixty5 package reaches hierarchy/search in 2.3 s and its first coarse frame in a 4.487 s three-run median ([record](../artifacts/ifc/sixty5-first-frame/README.md)). On the import side, the adapter needs 278.5 ms to produce the first document’s assembly tree and 40,474.9 ms to produce all seven ([record](../artifacts/import/structure-readiness/README.md)) | The compiled-package path is **Recorded**; the import path is **Designed** ([ADR-0021](adr/0021-staged-hierarchy-first-import.md)) and measured only as far as the adapter — publishing a tree to a viewer while import continues is **Pending** |
...
| Progressive cold-import preview | **Designed and measured** under proposed [ADR-0021](adr/0021-staged-hierarchy-first-import.md): the [structure-readiness record](../artifacts/import/structure-readiness/README.md) times an assembly tree on both federations — parsing is 98.5 percent of it, the containment walk 1.3 percent — so a whole-federation tree is 40,474.9 ms on sixty5 and cannot be brought inside the target by threads (the estimated six-thread makespan, 15,030.3 ms, equals the single largest document), while the first document is ready in 278.5 ms. The design publishes per document; nothing is implemented yet | Emit one document’s structure before tessellating anything and record the first published tree on sixty5 within the target (ADR-0021 gate 1) |
...
3. Publish hierarchy/search and coarse preview during that cold import inside
   ...
   sidecar and ADR-0020’s cancellation rules. Slices, in order: structure-first
   adapter emission, the staged package and its verified atomic publication, and
   the Studio background-import UI. The ADR’s gate 2 (a package digest
   identical with staging on and off) and gate 4 (a browser record of a usable
   tree during a cold sixty5 import) reject it on failure rather than relax it,
   and coarse preview stays explicitly unmeasured until its own gate 5.
