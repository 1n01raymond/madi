...
| WebGPU | multi-prototype `@naru3d/runtime-webgpu` scene plus a reproducible Chrome/Blink and Firefox/Gecko visual/picking matrix | Evidence currently covers one macOS/Apple-Silicon workstation, not the supported hardware matrix |
...
| Direct WebGPU works in two engines | Browser, OS, GPU, screenshot, adapter info, and picking result recorded | Passed on Chrome 151/Blink and Firefox 150/Gecko on macOS arm64; screenshots, metadata, and hashes are in `artifacts/browser-matrix` |
